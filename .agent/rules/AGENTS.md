# Go Project Rules & Agent Instructions

This file provides context and guidelines for GitHub Copilot when working on the `fuda` project.

## Project Overview

Fuda is a Go project.
...
- **Plan First:** Before modifying code, generate an "Implementation Plan" artifact. Wait for user acknowledgment for major architectural changes.
- **Small Diffs:** Break down large features into multiple smaller patches. Do not rewrite entire files if only one function changes.
- **Dependencies:** Check `go.mod` before adding new libraries. Prefer the standard library where possible.
...
- **Compile-time interface assertions:** Use strategically to avoid import cycles
  ...
  - **DO NOT use in:** Public subpackages (causes import cycles)
  ...
  - Example: `var _ fuda.Logger = (*NopLogger)(nil)` in `internal/logger/nop.go`
  - **Alternative:** Add interface tests in `_test.go` files for public subpackages:
    ```go
    func TestImplementsInterface(t *testing.T) {
        var _ fuda.SomeInterface = (*SomeType)(nil)
    }
    ```
...
var _ fuda.Logger = (*NopLogger)(nil)
...
- See `internal/state/state_test.go` for reference implementation
...
Run `make lint` to execute linter
